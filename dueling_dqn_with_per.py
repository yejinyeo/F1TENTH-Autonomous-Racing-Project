...
def train(q, q_target, memory, optimizer, beta=0.4):
    for _ in range(10):
        ...
        td_error = (q_a - target).detach().squeeze().abs().numpy()
        memory.update_priorities(indices.numpy(), td_error + 1e-5)
        ...
        while not done:
            ...
            if done:
                ...
                if lap_count == 2 and fastlap > lap:
                    torch.save(q.state_dict(), work_dir + '_' + RACETRACK + '/du_per_fast-model' + str(
...
def eval():
    current_dir = os.path.dirname(os.path.abspath(__file__))  # dpn.py가 있는 위치
    f1_root = os.path.abspath(os.path.join(current_dir, "../../../"))  # F1 루트 디렉토리
    ...
    q = Qnet()
    model_path = os.path.join(f1_root, "2025-06-15_16-05-29_Oschersleben", "du_per_fast-model69.52_6411.pt") #laptime: 111.17
